...
def _align_reward_to_frames(
    ...
    Returns:
        reward_full: (frame_num,) float32
        progress_full: (frame_num,) float32
    """
    ...
    # Handle rare non-monotonic timestamps
...
def _init_inference_objects(cfg, device: torch.device, camera_names: List[str]):
    """Initialize normalizer, CLIP encoder, and reward model once per process."""
    ...
    return state_normalizer, clip_encoder, reward_model
...
def _process_one_episode(
    ...
    """
    Process one episode directory:
    - loop over frames with eval_frame_gap
    - compute sampled delta-progress reward_list
    - align to per-frame reward using timestamp.npy
    - save reward.npy (per-frame) and progress.npy (per-frame)
    """
    ...
    for idx in range(0, frame_num, eval_frame_gap):
        ...
        if idx <= 500:
            raw_item = max(raw_item, 0.5)

        conf_val = 1.0  # placeholder (rewind model exposes no per-step confidence)
...
def label_reward_worker_entry(
    ...
    state_normalizer, clip_encoder, reward_model = _init_inference_objects(cfg, device, camera_names)
...
class ReWiNDLabelWorkspace:
    ...
    def label_reward(self):
        ...
        state_normalizer, clip_encoder, reward_model = _init_inference_objects(
            cfg, self.device, camera_names
        )
